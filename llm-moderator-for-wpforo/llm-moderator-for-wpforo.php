 ...
 * Tested with wpForo versions: 2.4.15
    ...
    if ( !$premium_plugin_active ) {
        ...
        <div class="notice notice-info is-dismissible" style="border-left-color: #0073aa; background: #f0f9ff; padding: 15px; margin: 20px 0;">
            ...
            <ul style="margin: 10px 0 15px 0;">
                ...
                <li>Charts and graph based metrics with user lookup</li>
            </ul>
            <p><strong>Get it now at: <a href="https://insertmywebsitehere.com/shop" target="_blank" style="color: #0073aa; text-decoration: underline;">insertmywebsitehere.com ( beta version, store coming soon )</a></strong></p>
...
function colaias_wpforo_ai_display_settings(){
    ...
    <div class="notice is-dismissible">
        <p style="font-size: small;"><strong style="color: green;">✅ Everything working?</strong> 🤖?? <strong style="color: red;">❌ No?</strong> ... 😅.
         If things break, revert to the tested wpForo version. Moreover, this is a <strong>Beta Version</strong>, bugs can be present (please let us know). Safety brakes should prevent crashes ( hopefully! 🤞 ). If wpForo updates break things, check debug.log 🕵️‍♂️. Use the tested version ( but even that might break mwhahaha 🦹 ) ⚠️ Small chance of memory leaks if not working ... Might need to deactivate ( not uninstall! 🚫🗑️ ).
        <br>
        <br>For urgent fixes 🚨, the fastest way to contact the maintainer is via <a href='https://t.me/wpforo_ai'>Telegram</a>. Note this down for future reference. 💬 Have your message ready! Not official wpForo devs 🚫👔 - indie developer. <strong>Contact us for bugs 🐛 or collabs 🤝 ONLY!</strong> NOT basic tech support for wpForo and WordPress 🙅‍♂️🙅‍♂️🙅‍♂️ 🤦‍♀️. Also, there isn't built in auto-bug-alert system 🚫 ( we may eventually make this plugin ). <strong>Community if things break, ALERT US! 📢</strong> Pro tip: Enable informational logging, when needed, to find out where the problem is occuring 🕵️‍♂️.
        <br>
        <br>❗ The "Admin" usergroup is needed by the automatic cleanup cron job to delete unapproved posts of muted users. Allow atleast 1 user in the "Admin" usergroup with all required Access permissions. Failure to do so will result in automatic cleanup not deleting unapproved posts. In addition, several features, such as running manual cleanup or giving usergroups unmute permissions, are only accessible by WordPress "Administrators". ℹ️ Click "Show forum details" in the settings below to see which permissions are needed.
            ...
            <table class="form-table">
                ...
                <tr>
                    <th scope="row">
                        ...
                        <small class="description" style="display: block; color: #6c757d;">⚠️ Note: The 'reason' key is recommended. It is used for flag metrics, to display the reason for muting user, and to append the LLM reason at the end of the user post with the {REASON} formatting tag.</small>
                        <br>
                    </th>
                    <td>
                        <?php 
            ...
            <div style="background: #fff; padding: 20px; border-radius: 5px; margin: 20px 0; border-left: 4px solid #0073aa;">
                ...
                <div style="display: grid; grid-template-columns: repeat( auto-fit, minmax( 250px, 1fr ) ); gap: 20px; margin: 20px 0;">
                    <div style="background: #f8f9fa; padding: 15px; border-radius: 5px; text-align: center;">
                        ...
                        <p style="font-size: 0.9em; color: #6c757d; margin: 5px 0 0 0;">Muted users' attempts to post</p>
                    </div>
                    <div style="background: #f8f9fa; padding: 15px; border-radius: 5px; text-align: center;">
 ...
 * Runs on a weekly schedule to minimize database impact
 */
function colaias_wpforo_ai_clean_old_flag_metrics() {
    try {
        global $wpdb;
        
        colaias_wpforo_ai_log_info( 'WPForo AI Moderation: Starting weekly cleanup of old flag metrics records' );
        ...
        if ( function_exists( 'WPF' ) ) {
            ...
            if ( $updated_post && $updated_post['status'] == $expected_status ) {
                ...
                if ( $updated_post ) {
                    ...
                    error_log( '🤖 ⚠️ WPForo AI Moderation: 2 ) ⏱️ ASYNC STATUS VERIFICATION ERROR / WARNING / FALSE POSITIVE' );
                }
            }
        }
    } catch ( Exception $e ) {
 ...
 */
function colaias_wpforo_ai_get_moderation_topic_content_with_context( &$topic, $max_succeeding ){
    try {
        // Use associative array as map since PHP doesn't have built-in Map class
        $topic_map = [];
        ...
        // Validate topic data
        if ( !isset( $topic['topicid'] ) || !isset( $topic['postid'] ) ) {
            colaias_wpforo_ai_log_info( "⚠️ WPForo AI Moderation: WARNING. Could not get topicid or postid. This is normal for a NEW topic. Returing empty for fallback" );
            return '';
        }
        ...
        // Add topic starter to map
        $first_post_content = sanitize_text_field( $topic['title'] . "\n\n" . $topic['body'] );
        ...
        $topic_map[$first_post_id]['relationships'][] = 'Topic starter post';

        // Get list of post IDs in the topic to find position and neighboring posts
        // Use our helper function for direct database query to get just post IDs
        // We're looking for the first post, so pass it as target_post_id
        $post_ids = colaias_wpforo_ai_get_topic_post_ids( $topic_id, $first_post_id, 0, $max_succeeding );
        
        if ( $post_ids && is_array( $post_ids ) && !empty( $post_ids ) ) {

            // Find first post position
            $first_post_index = array_search( $first_post_id, $post_ids );

            if ( $first_post_index !== false ) {
                // Get succeeding posts (posts after the first post in the same topic)
                if ( $max_succeeding > 0 ) {
                    $total_posts = count( $post_ids );
                    $end_index = min( $total_posts - 1, $first_post_index + $max_succeeding );

                    if ( $first_post_index < $total_posts - 1 ) {
                        // Get IDs of succeeding posts
                        $succeeding_ids = array_slice( $post_ids, $first_post_index + 1, $end_index - $first_post_index );

                        foreach ( $succeeding_ids as $index_offset => $succeeding_id ) {
                            $distance = $index_offset + 1;
                            $relationship = $distance == 1 ? 'Post succeeding directly (next post)' : "Post succeeding by {$distance} posts";

                            if ( $succeeding_id > 0 && !isset( $topic_map[$succeeding_id] ) ) {
                                $succeeding_post = WPF()->post->get_post( $succeeding_id, false );
                                if ( $succeeding_post ) {
                                    $succeeding_content = isset( $succeeding_post['body'] ) ? $succeeding_post['body'] : '';
                                    if ( isset( $succeeding_post['title'] ) && !empty( $succeeding_post['title'] ) ) {
                                        $succeeding_content = $succeeding_post['title'] . "\n\n" . $succeeding_content;
                                    }
                            ...
                            } else if ( $succeeding_id > 0 && isset( $topic_map[$succeeding_id] ) ) {
                                $topic_map[$succeeding_id]['relationships'][] = $relationship;
                            }
                        }
                    }
                }
        ...
        $json_string = json_encode( $topic_map, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE );

        // Handle JSON encoding errors
        if ( $json_string === false ) {
            // If encoding fails, return empty string
            colaias_wpforo_ai_log_info( '⚠️ ❌ 💀 WPForo AI Moderation: ERROR. JSON encoding failed. Returing empty for fallback' );
            return '';
        }
        
        colaias_wpforo_ai_log_info( 'WPForo AI Moderation: The encoded JSON topic CONTENT + CONTEXT string: ' . $json_string  );
        return $json_string;
    
    } catch (Exception $e) {
        // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log -- Critical error logging required for admin debugging
        error_log( '🤖 ❌ 💀 WPForo AI Moderation: CRITICAL ERROR. Could not get topic context for LLM. Exception : '
        .$e->getMessage() );
        return '';
    } 
}
...
/**
 * Premium function generously made available in base version. But set $max_ only through premium.
 * Returns organized and JSON string formated post content data with context for better moderation
 */
function colaias_wpforo_ai_get_moderation_post_content_with_context( $post, $max_succeeding, $max_preceding ){
    try {    
        // Use associative array as map since PHP doesn't have built-in Map class
        $post_map = [];
        ...
        if ( !isset( $post['topicid'] ) ) {
            colaias_wpforo_ai_log_info( "⚠️ ❌ 💀 WPForo AI Moderation: ERROR. UNUSUAL. When getting CONTEXT. Could not validate topicid {$post['topicid']}. Returing empty for fallback" );
            return '';
        }

        // Validate post data
        if ( !isset( $post['postid'] ) ) {
            colaias_wpforo_ai_log_info( "⚠️ WPForo AI Moderation: Could not validate post ID, likely means the post is a reply, setting post ID to 0" );
            $post['postid'] = '0';
        }

        $post_id = $post['postid']; // this doesn't exist for the NEW post, returns 0 For new post, or null for new reply post.
        $topic_id = $post['topicid'];

        // Get the topic
        $topic = WPF()->topic->get_topic( $topic_id, false );
        if ( !$topic ) {
            colaias_wpforo_ai_log_info( '⚠️ ❌ 💀 WPForo AI Moderation: ERROR. UNUSUAL. When getting CONTEXT. Could not get topic. Returing empty for fallback' );
            return '';
        }
        ...
        $current_post_content = isset( $post['body'] ) ? sanitize_text_field( $post['body'] ) : '';
        if ( isset( $post['title'] ) && !empty( $post['title'] ) ) {
            $current_post_content = $post['title'] . "\n\n" . $current_post_content;
        }
        ...
        // Get the first post (topic starter)
        $first_post_id = isset( $topic['first_postid'] ) ? $topic['first_postid'] : null;
        if ( $first_post_id ) {
            $first_post = WPF()->post->get_post( $first_post_id, false );
            if ( $first_post ) {
                // Add topic starter to map
                if ( ! isset( $post_map[$first_post_id] ) ) {
                    ...
                    $post_map[$first_post_id]['relationships'][] = 'Topic starter post';
                }
            }
        }

        // Check if this post is a reply to another post
        // In wpForo, if a post has a parentid > 0, it's a reply to that post
        $parent_id = isset( $post['parentid'] ) ? $post['parentid'] : 0;
        if ( $parent_id > 0 ) {
            $parent_post = WPF()->post->get_post( $parent_id, false );
            if ( $parent_post ) {
                // Check if parent already exists in map
                if ( isset( $post_map[$parent_id] ) ) {
                    // Append relationship
                    $post_map[$parent_id]['relationships'][] = 'Post being replied to';
                } else {
                    // Add parent to map
                    $parent_content = isset( $parent_post['body'] ) ? $parent_post['body'] : '';
                    if ( isset( $parent_post['title'] ) && !empty( $parent_post['title'] ) ) {
                        $parent_content = $parent_post['title'] . "\n\n" . $parent_content;
                    }
                    $post_map[$parent_id] = [
        ...
        // Get list of post IDs in the topic to find position and neighboring posts
        // Use our helper function for direct database query to get just post IDs
        // We're looking for the current post, so pass it as target_post_id
        $post_ids = colaias_wpforo_ai_get_topic_post_ids( $topic_id, $post_id, $max_preceding, $max_succeeding );
        
        if ( $post_ids && is_array( $post_ids ) && !empty( $post_ids ) ) {

            // Find current post position
            if ($post_id == 0) {
                colaias_wpforo_ai_log_info( "📊 WPForo AI Moderation: post id is 0, likely means NEW post. current index is set to length of post_ids for context moderation" );
                $current_index = count($post_ids);
            } else {
                $current_index = array_search( $post_id, $post_ids );
            }

            if ( $current_index !== false ) {
                // Get preceding posts (posts before this one in the same topic)
                if ( $max_preceding > 0 && $current_index > 0 ) {
                    $start_index = max( 0, $current_index - $max_preceding );

                    // Get IDs of preceding posts
                    $preceding_ids = array_slice( $post_ids, $start_index, $current_index - $start_index );

                    foreach ( $preceding_ids as $index_offset => $preceding_id ) {
                        $distance = $current_index - ( $start_index + $index_offset );
                        $relationship = $distance == 1 ? 'Post preceding directly (previous post)' : "Post preceding by {$distance} posts";

                        if ( $preceding_id > 0 && !isset( $post_map[$preceding_id] ) ) {
                            $preceding_post = WPF()->post->get_post( $preceding_id, false );
                            if ( $preceding_post ) {
                                $preceding_content = isset( $preceding_post['body'] ) ? $preceding_post['body'] : '';
                                if ( isset( $preceding_post['title'] ) && !empty( $preceding_post['title'] ) ) {
                                    $preceding_content = $preceding_post['title'] . "\n\n" . $preceding_content;
                                }
                        ...
                        } else if ( $preceding_id > 0 && isset( $post_map[$preceding_id] ) ) {
                            $post_map[$preceding_id]['relationships'][] = $relationship;
                        }
                    }
                }

                // Get succeeding posts (posts after this one in the same topic)
                if ( $max_succeeding > 0 ) {
                    $total_posts = count( $post_ids );
                    $end_index = min( $total_posts - 1, $current_index + $max_succeeding );

                    if ( $current_index < $total_posts - 1 ) {
                        // Get IDs of succeeding posts
                        $succeeding_ids = array_slice( $post_ids, $current_index + 1, $end_index - $current_index );

                        foreach ( $succeeding_ids as $index_offset => $succeeding_id ) {
                            $distance = $index_offset + 1;
                            $relationship = $distance == 1 ? 'Post succeeding directly (next post)' : "Post succeeding by {$distance} posts";
                            if ( $succeeding_id > 0 && !isset( $post_map[$succeeding_id] ) ) {
                                $succeeding_post = WPF()->post->get_post( $succeeding_id, false );
                                if ( $succeeding_post ) {
                                    $succeeding_content = isset( $succeeding_post['body'] ) ? $succeeding_post['body'] : '';
                                    if ( isset( $succeeding_post['title'] ) && !empty( $succeeding_post['title'] ) ) {
                            ...
                            } else if ( $succeeding_id > 0 && isset( $post_map[$succeeding_id] ) ) {
                                $post_map[$succeeding_id]['relationships'][] = $relationship;
                            }
                        }
                    }
                }
        ...
        // Convert the map to JSON string
        $json_string = json_encode( $post_map, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE );
        ...
        colaias_wpforo_ai_log_info( 'WPForo AI Moderation: The encoded JSON post CONTENT + CONTEXT string:   ' . $json_string  );
        return $json_string;
    } catch (Exception $e) {
         // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log -- Critical error logging required for admin debugging
         error_log( '🤖 ❌ 💀 WPForo AI Moderation: CRITICAL ERROR. Could not get post context for LLM. Exception : '
         .$e->getMessage() );
        return '';
    }
}
 ...
 * @param mixed $prompt  Administrator settings tab prompt
 */
